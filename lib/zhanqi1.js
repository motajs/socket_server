...
module.exports = function (io, tower_name, log) {
    ...
    function sendResult(winner, loser, tie, id) {
        log(id + " competition result: " + winner.uid + " vs " + loser.uid + " " + (tie ? "tie" : "win") + "!");
        var data = {
            name: tower_name,
        ...
        request.post({
            url: "https://h5mota.com/rank/uploadrank.php",
            form: data
    ...
    tower.on('connection', function (socket) {
        ...
        socket.on('ready', function (id, uid, password) {
            var room = tower.adapter.rooms[id];
            if (!room) {
        ...
        socket.on('disconnecting', function () {
            ...
            if (id == null) return;
            tower.in(id).emit('disconnect', '对方断开了链接');
        ...
        socket.on('result', function (id, result) {
            ...
            var _check = function () {
                if (room.winner && room.loser) {
                    sendResult(room.winner, room.loser, 0, id);
                    room.started = false;
                }
            }
            ...
            if (result == 2) {
                if (room.tie == null) room.tie = player;
                    ...
                    sendResult(room.tie, player, 1, 1);
                    room.started = false;
                }
                return;
            }
        ...
        ["put", "msg", "asktie", "asktie_res", "asklose"].forEach(function (e) {
